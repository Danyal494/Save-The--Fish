...
const Fish = () => {
    ...
    return setInterval(() => {
      setFill((prevFill) => {
        ...
        if (newFill <= 0) {
          ...
          fishRef.current?.classList.remove('bowl_fish--dead', 'bowl_fish--dying');
        }
        return newFill;
      });
    }, 200);
  ...
  const handleTapClick = () => {
    ...
    setTimeout(() => tapRef.current?.classList.remove('bowltap--active'), 500);
    if (fill <= 0) {
      intervalId.current = emptyingFn();
